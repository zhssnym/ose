...
import { esc, openOverlay, toast } from 'ose:ui';

const { store, commands } = ose;
// Any key flushes the whole state file; this one is the shell's own.
...
export function updatesEnabled() { return ose.settings.get().updates !== false; }
...
export function initUpdate() {
  ...
  // (no CI stamp, `sha` of 'dev') is known at once and never scheduled.
  const v = ose.version || {};
  build = v.sha && v.sha !== 'dev' ? { short: v.short, sha: v.sha, date: v.date } : null;
  reschedule();
}
