...
const claudeEvents = [];
bridge.on('claude', (d) => claudeEvents.push(d));
...
async function run() {
  ...
  let cli = null, id = null;
  await test('claudeInfo', async () => {
    cli = await bridge.claudeInfo();
    assert(cli && typeof cli === 'object', 'bad shape: ' + JSON.stringify(cli));
    // A machine without the CLI (CI runners) is a skip, not a failure: the command itself worked.
    return cli.path ? `${cli.path} ${cli.version || ''}` : 'no claude CLI on this machine';
  });

  if (cli && cli.path) {
    await test('claudeStart', async () => {
      const r = await bridge.claudeStart({ cwd: '', permissionMode: 'default' });
      assert(r && r.id, 'no session id: ' + JSON.stringify(r));
      id = r.id;
      return 'id=' + id;
    });
  } else {
    for (const n of ['claudeStart', 'claudeSend + result', 'claudeStop']) skipped(n, 'claude CLI not found');
  }

  if (id) {
    await test('claudeSend + result within 90s', async () => {
      const before = claudeEvents.length;
      await bridge.claudeSend(id, 'Reply with exactly the word pong and nothing else.');
      const deadline = Date.now() + CLAUDE_WAIT_MS;
      while (Date.now() < deadline) {
        const hit = claudeEvents.find((e) => e.id === id && e.event && e.event.type === 'result');
        if (hit) {
          const r = hit.event;
          return `${claudeEvents.length - before} events, is_error=${r.is_error}, ${r.duration_ms}ms, text=${trim(r.result, 60)}`;
        }
        await sleep(250);
      }
      const seen = [...new Set(claudeEvents.map((e) => e.event && e.event.type))].join(',');
      throw new Error(`no result event in ${CLAUDE_WAIT_MS}ms; saw: ${seen || 'nothing'}`);
    });
    await test('claude event types', async () => [...new Set(claudeEvents.map((e) => e.event && e.event.type))].join(',') || 'none');
    await test('claudeStop', async () => { await bridge.claudeStop(id); return 'killed'; });
  }
