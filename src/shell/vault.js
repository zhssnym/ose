...
export async function recentVaults() {
  try {
    const list = await bridge.recentVaults();
...
export async function chooseVault() {
  ...
  if (!list.length) return bridge.pickVault();
  ...
  return new Promise((resolve) => {
    ...
    ov.box.addEventListener('click', async (e) => {
      ...
      if (e.target.closest('[data-act="pick"]')) {
        try { finish(await bridge.pickVault()); } catch (err) { finish(null); console.error('[shell] pickVault', err); }
        return;
      }
      const row = e.target.closest('.vault-row');
      if (!row) return;
      try {
        finish(await bridge.openVault(row.dataset.path));
      } catch (err) {
    ...
    bindRowKeys(ov.box, {
      onForget: async (path) => {
        try { await bridge.forgetVault(path); } catch (err) { console.warn('[shell] forgetVault', err); }
...
export function vaultLost(root) {
  ...
  retry.addEventListener('click', async () => {
    retry.disabled = true;
    let back = false;
    try { const st = await bridge.stat(''); back = !!(st && st.exists); } catch { back = false; }
...
export async function mountVaultChooser(rootEl) {
  ...
  list.addEventListener('click', async (e) => {
    ...
    err.hidden = true;
    try {
      const opened = await bridge.openVault(row.dataset.path);
  ...
  bindRowKeys(body, {
    onForget: async (path) => {
      try { await bridge.forgetVault(path); } catch (e) { console.warn('[shell] forgetVault', e); }
      items = items.filter((v) => v.path !== path);
      paintRecent();
