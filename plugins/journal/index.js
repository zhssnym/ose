...
async function renderTo(target, before = null) {
  ...
  await Promise.all(need.map(async (d) => {
    try { d.text = await ose.files.read(d.path); }
    catch (e) { console.warn('[journal] read', d.path, e); d.text = ''; }
...
async function save() {
  ...
  try {
    ...
    if (await ose.files.exists(path)) {
      const cur = await ose.files.read(path);
      const gap = cur.endsWith('\n\n') ? '' : cur.endsWith('\n') ? '\n' : '\n\n';
      await ose.files.append(path, `${gap}---\n\n${text}\n`);
    } else {
      await ose.files.write(path, `${journalHeading(now)}\n\n${text}\n`);
    }
    ta.value = '';
    persistNow('');
    grow(ta);
    syncSave();
    ose.status.set('save', `saved ${hhmm(new Date())}`);

    // splice today into the record in place: one small read, no rebuild from scratch
    const fresh = await ose.files.read(path);
    ...
    ose.status.set('save', 'save failed');
  }
  syncSave();
}
...
const view = {
  ...
  async mount(host) {
    ...
    const saved = ose.state('mode').get();
    mode = MODES.includes(saved) ? saved : 'full';
    drawMode();
...
export async function activate(app) {
  ose = app;
  ose.views.register('journal', view);
  ...
  ose.commands.register({
    ...
    run: () => {
      ...
      view.focusComposer();
      // the view mounts asynchronously; ask again on the next frames
      requestAnimationFrame(() => view.focusComposer());
      setTimeout(() => view.focusComposer(), 120);
    },
  });
