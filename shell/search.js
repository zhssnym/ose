...
// arrows move, Enter opens, Esc closes. `ose.search` runs debounced 150ms as you type.
//
...
import { esc, icon, openOverlay } from 'ose:ui';
import { revealFolder } from './sidebar.js';
import { titleOf, dirName } from './paths.js';

const { commands, debounce } = ose;
const navigate = (r, opts) => ose.route.navigate(r, opts);
...
export function openSearch(opts = {}) {
  ...
  const run = debounce(async () => {
    ...
    try {
      ...
      const r = await ose.search(q, { limit: getFocus() ? FOCUS_LIMIT : LIMIT, chan: 'overlay' });
